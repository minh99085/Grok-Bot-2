#!/usr/bin/env python3
"""Validate .env against soak/learning frozen manifest. Prints JSON; exit 1 on drift."""
...
VPS_ENV = Path("/opt/Grok-Bot-2/hermes-agent-main/plugins/hermes-trading-engine/.env")
        ...
        for key, want in (manifest.get(section) or {}).items():
            ...
            if got != want:
                ...
                issues.append(_issue(
                    ...
                    "run scripts/apply-loop-arch-env.py; see .grok/rules/soak-learning-lock.md",
                ))
