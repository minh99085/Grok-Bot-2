...
def test_directional_halted_on_low_profit_factor():
    ...
    assert out["reasons"]  # wilson and/or profit-factor breach
...
def test_arb_halted_on_non_positive_guaranteed_profit():
    cfg = StopConfig()
    out = evaluate_arbitrage(
        arb_positions={"w1": {"guaranteed_profit_usd": 0.0, "status": "open"}},
        arb_report={"executed": 1, "settled": 0, "realized_profit_usd": 0},
        cfg=cfg)
    assert out["halted"] is True
    assert "non_positive_guaranteed_profit" in out["reasons"]
