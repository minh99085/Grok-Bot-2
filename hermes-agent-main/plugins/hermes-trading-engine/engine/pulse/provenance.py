...
def _env_proof() -> dict:
    keys = (
        ...
        "PULSE_DIRECTIONAL_REQUIRE_WINNING", "PULSE_LEARNING_ENABLED", "PULSE_ARB_ENABLED",
...
def build_validation_full(light_report: dict, status: Optional[dict] = None) -> str:
    ...
    lines = [
        ...
        "arbitrage=%s" % json.dumps({k: (lr.get("arbitrage") or {}).get(k) for k in
                                     ("executed", "settled", "realized_profit_usd", "segregated_from_directional")}),
